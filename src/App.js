...
const LCDScreen = ({ settings, batteryPercentage }) => {
  const {
  ...
  return (
    <div className="relative w-full h-full bg-gray-800 text-white rounded-lg p-4 font-mono text-xs flex flex-col justify-between overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full p-2"> {/* 移除 overflow-y-auto custom-scrollbar */}
        ...
        {/* 調整後的電量、SD卡、日期時間顯示區塊 */}
        <div className="text-xl mb-3 flex justify-between items-center w-full flex-wrap"> {/* Adjusted font size to text-xl */}
            ...
            <span>電量: {batteryPercentage}%</span> {/* Moved Battery third and display percentage */}
        </div>
  ...
  return (
    <div className="p-6 bg-gray-700 rounded-lg shadow-inner w-full overflow-y-auto custom-scrollbar h-full">
      ...
      <div className="border-b border-gray-600 pt-4 mb-4">
        <h3 className="text-lg font-bold text-blue-200 mb-2">時間調整 (Setup DATE / TIME)</h3> {/* Changed title */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 mb-4">
          <div>
            <label htmlFor="year" className="block text-gray-200 text-sm font-bold mb-1">
              年<br />(Year)
            </label>
          ...
          <div>
            <label htmlFor="month" className="block text-gray-200 text-sm font-bold mb-1">
              月<br />(Month)
            </label>
          ...
          <div>
            <label htmlFor="day" className="block text-gray-200 text-sm font-bold mb-1">
              日<br />(Day)
            </label>
          ...
          <div>
            <label htmlFor="hour" className="block text-gray-200 text-sm font-bold mb-1">
              時<br />(Hour)
            </label>
          ...
          <div>
            <label htmlFor="minute" className="block text-gray-200 text-sm font-bold mb-1">
              分<br />(Minute)
            </label>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {/* 拍攝定時器 */}
        <div className="border-t border-gray-600 pt-4 mt-4">
          <h3 className="text-lg font-bold text-blue-300 mb-2">拍攝定時器 (Capture Timer)</h3>
          <div>
...
const App = () => {
  ...
  // 新增：每秒更新 currentDate 狀態並每5秒更新電池狀態
  useEffect(() => {
    const timerId = setInterval(() => {
      setSettings(prevSettings => {
        ...
        // 每5秒減少1%電量
        if (newDate.getSeconds() % 5 === 0) {
          setBatteryPercentage(prevBatt => Math.max(0, prevBatt - 1));
        }
  ...
  return (
    // 將整個應用程式的背景設定為 LCD 的深灰色
    <div className="flex flex-col min-h-screen bg-gray-800 font-inter text-white">
      <h1 className="text-3xl font-bold text-gray-200 mb-2 mt-4 text-center">Browning 紅外線相機模擬器</h1>
      <p className="text-sm text-gray-400 mb-8 text-center">臺北醫學大學 製作</p>

      {/* 總容器，用於水平排布 LCD 和設定面板 */}
      <div className="flex flex-1 w-full p-4 gap-6 overflow-hidden">
        {/* 左側：LCD 螢幕區，佔據大部分空間 */}
        <div className="flex-grow bg-gray-600 rounded-3xl shadow-2xl p-6 border-4 border-gray-700 overflow-hidden">
          <LCDScreen settings={settings} batteryPercentage={batteryPercentage} />
        </div>

        {/* 右側：設定控制面板，固定寬度 */}
        <div className="flex-shrink-0 w-80 md:w-96 max-h-full">
